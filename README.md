...
•  The program accesses a relational database to manage (add, update, delete) appointments and customer records. The login screen can be translated into French depending on the end-user's computer language setting.  A user must successfully provide the correct login username and password from the database to access the application's main screen. All successful and unsuccessful user log-in attempts are recorded. The appointments from the database, which are in Coordinated Universal Time (UTC), are converted to the user's local time zone and displayed in the application. There are up to three time zones in effect. Coordinated Universal Time (UTC) is used for storing the time in the database, the user’s local time is used for display purposes, and Eastern Standard Time (EST) is used for the company’s office hours. Local time will be checked against EST business hours before they are stored in the database as UTC. If there is an appointment within 15 minutes of the user's login, an alert will pop up. There are error checks for appointments outside of business hours. scheduling overlapping appointments, and entering an incorrect username and password. Updating a customer or appointment record, the record's data autopopulates the update form. Country and first-level division data, which is found on the Add Customer or Modify Customer form, is prepopulated in separate combo boxes or lists in the user interface for the user to choose. The first-level list is filtered by the user’s selection of a country or vice-versa. The application allows a user to generate a report about all appointments associated with a customer or a contact in the organization, and the total number of appointments by type and month.
